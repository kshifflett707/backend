...
export const updatePostVote = () => (req, res) => {
  const data = req.body
  const id = req.params.id
  if (data.type === 'up') {
    Post.increment('upvoteCount', {where: {id}})
    .then((post) => res.status(200).json(post))
    .catch((err) => res.status(404).json(err))
  } else {
    Post.decrement('upvoteCount', {where: {id}})
    .then((post) => res.status(200).json(post))
    .catch((err) => res.status(404).json(err))
  }
}

export const answersById = () => (req, res) =>
  Post.getPostsByQuery({PostId: req.params.id, limitBy: 50})
  .then((posts) => res.status(200).json(posts))
  .catch((err) => res.status(401).json({err}));
